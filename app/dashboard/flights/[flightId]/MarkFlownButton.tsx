...
export default function MarkFlownButton({
  ...
  async function handleMark() {
    setLoading(true)
    try {
      const res = await fetch(`/api/flights/${flightId}`, { method: 'PATCH' })
  ...
  if (confirming) {
    return (
      <div className="flex items-center gap-2">
        <span className="text-sm text-gray-400">Mark as flown?</span>
        <button
  ...
  return (
    <button
      ...
      className="bg-dark-elevated hover:bg-dark-border text-gray-400 hover:text-[#F1F2F4] border border-dark-border px-4 py-2.5 rounded-md transition-colors text-sm font-medium flex items-center gap-2"
    >
      <CheckCircle size={15} />
      Mark as flown
    </button>
